        ...
        foreach ($steps as &$step) {
            ...
            if ($exists) {
                Piwik_Query("UPDATE " . Piwik_Common::prefixTable('funnel_step') . "
                             ...
                             deleted = 0
                             WHERE idsite = ? AND idstep = ? AND idfunnel = ?", array($name, $url, $match_attribute, $pattern_type, $case_sensitive, $idSite, $idStep,
                    $idFunnel));
            } else {
                Piwik_Query("INSERT INTO " . Piwik_Common::prefixTable('funnel_step') . "
							 ...
							 VALUES (?, ?, ?, ?, ?, ?, ?, ?)", array($idSite, $idFunnel, $idStep, $name, $url, $match_attribute, $pattern_type,
                    $case_sensitive));
            }
        }
